...
import {compareDataOnSetBinding, deepComparatorTokenBinding, defaultCacheTtlBinding} from "./binding";
...
export class CacheController<E = ErrorData> {
    ...
    private s = new Subject<{ requestKey: string, value: RequestState<any, E> }>();
    private readonly requestCache = new Map<string, CachedResponse<unknown, unknown>>();

    constructor(
        @Inject(compareDataOnSetBinding) compareDataOnSet: boolean,
        @Inject(deepComparatorTokenBinding) deepComparator: typeof deepObjectCompare,
    ...
    public subscribe<T>(requestKey: string, observerOrNext?: Partial<Observer<RequestState<T, E>>> | ((value: RequestState<T, E>) => void)) {
        return this.s.pipe(
            filter((value) => value.requestKey === requestKey),
            map((value) => value.value),
            filter((value): value is RequestState<T, E> => value !== undefined)
        )
            .subscribe(observerOrNext);
    }

    public set<T>(requestKey: string, data: RequestState<T, E>, expiry = this.defaultCacheTtl) {
        ...
        if (this.compareDataOnSet) {
            ...
            if (!!cache) {
                ...
                flagsIsEqual = cache.error === data.error
    ...
    public merge<T>(requestKey: string, data: Partial<RequestState<T, E>>, cacheTtl = this.defaultCacheTtl) {
        const cache = this.get(requestKey) ?? {
            data: undefined,
            error: undefined,
            ...inProgressState
